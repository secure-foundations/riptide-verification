...
ANSI.in_<code>(x) will output a ANSI formatted string
which, when printed on the terminal, will have style <code>
If the tool detects that the terminal does not support ANSI,
the same string x is returned

<code> ::= <color> # set foreground color
         | <style>_<color> # set foreground color and style
         | <color>_<color> # set foreground and background colors

e.g. ANSI.in_bold_grey
"""
...
class ANSIStyler(type):
    ...
    def __getattr__(class_obj, key: str) -> Any:
        if key.startswith("in_"):
            ...
            styles = key[len("set_"):].split("_")

            def f() -> str:
                if ANSI.supports_color():
                    return class_obj.get_code(styles)
                return ""
