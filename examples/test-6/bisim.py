    ...
    def branch(results: Tuple[dataflow.StepResult, ...], new_trace_counter: int):
        ...
        for branch in branches:
            ...
            if check_implication(
                ...
                assert False, f"bad branch: LLVM branch path condition {branch.config.path_conditions}, dataflow path condition: {results[0].config.path_conditions}"
...
def main():
    """
    See examples/bisim/test-6.png for operator and channel IDs in the dataflow graph
    """

    with open("examples/test-6/test-6.o2p") as dataflow_source:
    ...
    with open("examples/test-6/test-6.ll") as llvm_source:
        ...
        llvm_loop_config = llvm.Configuration(
            module,
            function,
    ...
    # correspondence between LLVM variables and dataflow variables (excluding parameters and memory)
    var_correspondence = [
        (),
        None, # to be inferred
    ]
