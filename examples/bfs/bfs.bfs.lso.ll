...
define dso_local void @bfs(i32* noalias nocapture readonly %rows, i32* noalias nocapture readonly %cols, i32 %count, i32* nocapture %queue, i32* nocapture %visited, i32* noalias nocapture %walk) local_unnamed_addr #0 {
entry:
  br label %while.cond

while.cond.loopexit:                              ; preds = %for.cond
  %lso.alloc2.1.lcssa = phi i32 [ %lso.alloc2.1, %for.cond ]
  %lso.alloc.1.lcssa = phi i32 [ %lso.alloc.1, %for.cond ]
...
while.cond:                                       ; preds = %while.cond.loopexit, %entry
  ...
  %lso.alloc.0 = phi i32 [ 0, %entry ], [ %lso.alloc.1.lcssa, %while.cond.loopexit ]
...
while.body:                                       ; preds = %while.cond
  ...
  %1 = load i32, i32* %arrayidx3, align 4, !tbaa !6
  %add = add nsw i32 %0, 1
...
for.cond:                                         ; preds = %if.end, %while.body
  ...
  %lso.alloc.1 = phi i32 [ %lso.alloc.0, %while.body ], [ %lso.alloc.2, %if.end ]
  %i.0 = phi i32 [ %1, %while.body ], [ %inc12, %if.end ]
...
for.body:                                         ; preds = %for.cond
  ...
  %arrayidx7 = getelementptr inbounds i32, i32* %visited, i32 %3
  %4 = call i32 (i32*, ...) @cgra_load32(i32* %arrayidx7, i32 %lso.alloc.1)
  %tobool.not = icmp eq i32 %4, 0
  br i1 %tobool.not, label %if.then, label %if.end

if.then:                                          ; preds = %for.body
  %arrayidx8 = getelementptr inbounds i32, i32* %queue, i32 %queue_back.1
  %5 = call i32 (i32, i32*, ...) @cgra_store32(i32 %3, i32* %arrayidx8)
  %inc10 = add nsw i32 %queue_back.1, 1
  %6 = call i32 (i32, i32*, ...) @cgra_store32(i32 1, i32* %arrayidx7, i32 %5)
  br label %if.end

if.end:                                           ; preds = %if.then, %for.body
  %lso.alloc2.2 = phi i32 [ %6, %if.then ], [ %lso.alloc2.1, %for.body ]
  %lso.alloc.2 = phi i32 [ %6, %if.then ], [ %lso.alloc.1, %for.body ]
